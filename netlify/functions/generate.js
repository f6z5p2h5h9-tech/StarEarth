// Netlify Serverless Function: AI Course Outline Generator
...
exports.handler = async function(event, context) {
  ...
  if (!apiKey) {
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ error: '未配置 DEEPSEEK_API_KEY 环境变量。请在 Netlify 后台 Site settings → Environment variables 中添加。' })
    };
  }

  try {
    const { idea, age, modules, courseType, pblCases } = JSON.parse(event.body);
...
**课程类型：** ${courseType}`;
    ...
    if (!response.ok) {
      ...
      return {
        ...
        headers,
        body: JSON.stringify({ error: errData.error?.message || `DeepSeek API 错误 (${response.status})` })
      };
    }
