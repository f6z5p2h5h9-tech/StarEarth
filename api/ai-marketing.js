// Vercel Serverless Function: AI Marketing Content Generator
...
- 朋友圈专属文案调性：
  ...
  2. 【绝对杜绝营销词汇】：禁止使用任何如“震撼来袭”、“赶紧抢位”、“宝妈快看”、“星与土带你...”等生硬的营销硬广词汇，也绝对不要像小红书那样使用一堆堆砌的夸张修饰语。它必须是克制、真实、充满真情实感、细腻且高级的日常记录。
  3. 【排版美学】：多用换行与空行制造“空气感”和呼吸感，禁止使用话题标签（#），朋友圈没有标签。Emoji 每篇仅点缀 2-3 个即可，避免杂乱。
...
export default async function handler(req, res) {
  ...
  try {
    const { idea, platform, tone, courseInfo, category } = req.body;
...
**语气风格：** ${tone || '温暖真实'}`;
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: errData.error?.message || `API 错误 (${response.status})` });
    }
