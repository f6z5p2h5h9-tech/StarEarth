// Vercel Serverless Function: AI Course Outline Generator
// Proxies requests to DeepSeek API with StarEarth educational context
...
5. **走出舒适区**：任务难度应略高于学生当前能力水平，设计需要勇气 and 冒险精神才能完成的环节。
...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    return res.status(500).json({ error: '未配置 DEEPSEEK_API_KEY 环境变量。请在 Vercel 后台添加。' });
  }

  try {
    const { idea, age, modules, courseType, pblCases } = req.body;

    // Build user message
    let userMsg = `请根据以下信息生成一份符合星与土教育理念的课程大纲：
...
**课程类型：** ${courseType}`;
    ...
    // Include PBL cases from localStorage if provided
    if (pblCases && pblCases.length > 0) {
      userMsg += `\n\n**已有的 PBL 案例参考（来自案例库）：**\n`;
      pblCases.slice(0, 3).forEach((c, i) => {
        userMsg += `\n案例${i + 1}：${c.title}（${c.age}）\n- 驱动问题：${c.eq}\n- 亮点：${c.highlights}\n`;
      });
    }
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({ error: errData.error?.message || `DeepSeek API 错误 (${response.status})` });
    }
